...
public class StreamsPresenter implements StreamsContract.Presenter {
   ...
   */
  StreamsPresenter(StreamsContract.View streamsView) {
  ...
  private ConnectionEventCallback connectionEventCallback = new ConnectionEventCallback() {
    ...
    public void failedToConnect(@NotNull Isometrik isometrik,
      ...
      streamsView.failedToConnect(connectionFailedEvent.getReason());
    }
  };
  ...
  /**
   * {@link StreamsContract.Presenter#registerConnectionEventListener()}
   */
  @Override
  ...
  /**
   * {@link StreamsContract.Presenter#unregisterConnectionEventListener()}
   */
  @Override
  ...
  /**
   * {@link StreamsContract.Presenter#requestLiveStreamsData(int, boolean)}
   */
  @Override
  public void requestLiveStreamsData(int pageSize, boolean refreshRequest) {
    ...
    isometrik.fetchStreams(new FetchStreamsQuery.Builder().setCount(pageSize)
      ...
      if (var1 != null) {
        //if (isometrik.getConfiguration().getRemoteMessageVerbosity()
        //    == IMRemoteMessageVerbosity.FULL) {
        //  Log.i(Isometrik.getIsometrikSuccessTag(), var1.getMessage());
        //}
        ...
        streamsView.onLiveStreamsDataReceived(streamsModels, refreshRequest);
      } else {

        //if (isometrik.getConfiguration().getRemoteMessageVerbosity()
        //    == IMRemoteMessageVerbosity.FULL) {
        //  Log.i(Isometrik.getIsometrikErrorTag(), "Remote error-" + var2.isRemoteError());
        //  Log.i(Isometrik.getIsometrikErrorTag(), "Error message-" + var2.getErrorMessage());
        //  Log.i(Isometrik.getIsometrikErrorTag(),
        //      "Http response code-" + var2.getHttpResponseCode());
        //  Log.i(Isometrik.getIsometrikErrorTag(),
        //      "Remote error code" + var2.getRemoteErrorCode());
        //  Log.i(Isometrik.getIsometrikErrorTag(),
        //      "Isometrik error code" + var2.getIsometrikErrorCode());
        //}
        if (var2.getHttpResponseCode() == 404 && var2.getRemoteErrorCode() == 1) {

          if (refreshRequest) {
            //No streams found
            streamsView.onLiveStreamsDataReceived(new ArrayList<>(), true);
          } else {
            isLastPage = true;
          }
        } else {
  ...
  /**
   * {@link StreamsContract.Presenter#requestLiveStreamsDataOnScroll(int, int, int)}
   */
  @Override
  ...
  /**
   * {@link StreamsContract.Presenter#addSubscription(boolean)}
   */
  @Override
  public void addSubscription(boolean streamStartEvents) {
    isometrik.addSubscription(
        new AddSubscriptionQuery.Builder().setStreamStartChannel(streamStartEvents)
  ...
  /**
   * {@link StreamsContract.Presenter#removeSubscription(boolean)}
   */
  @Override
  public void removeSubscription(boolean streamStartEvents) {
    isometrik.removeSubscription(
  ...
  /**
   * {@link StreamsContract.Presenter#updateUserPublishStatus()}
   */
  @Override
  public void updateUserPublishStatus() {
    isometrik.updateUserPublishingStatus(new UpdateUserPublishingStatusQuery.Builder().setUserId(
        IsometrikUiSdk.getInstance().getUserSession().getUserId()).build(), (var1, var2) -> {
