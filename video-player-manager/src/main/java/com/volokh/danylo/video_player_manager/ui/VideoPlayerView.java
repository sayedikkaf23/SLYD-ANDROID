 ...
 * It encapsulates {@link IjkMediaPlayer}.
 * <p>
 * It ensures that MediaPlayer methods are called from not main thread.
 * MediaPlayer methods are directly connected with hardware. That's why they should not be called
 * from UI thread
 *
 * @author danylo.volokh
 */
public class VideoPlayerView extends ScalableTextureView
        implements TextureView.SurfaceTextureListener, MediaPlayerWrapper.MainThreadMediaPlayerListener,
     ...
     * If you need to use it you should synchronize in on {@link VideoPlayerView#mReadyForPlaybackIndicator}
    ...
    /**
     * A Listener that propagates {@link IjkMediaPlayer} listeners is background thread.
     * Probably call of this listener should also need to be synchronized with it creation and destroy
     * places.
     */
    ...
    public void setDataSource(String path,boolean videoMuted) {
        ...
        synchronized (mReadyForPlaybackIndicator) {
            ...
            try {
                mMediaPlayer.setDataSource(path,videoMuted);
            } catch (IOException e) {
    ...
    private final Runnable mVideoCompletionBackgroundThreadRunnable = new Runnable() {
        @Override
        public void run() {
            mMediaPlayerListenerBackgroundThread.onVideoSizeChangedBackgroundThread(getContentHeight(),
                    getContentWidth());
        }
    };
    ...
    private void notifyOnVideoRenderingStartedMainThread() {
        //if (SHOW_LOGS) Logger.v(TAG, "notifyOnVideoRenderingStartedMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners) {
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
        for (MediaPlayerWrapper.MainThreadMediaPlayerListener listener : listCopy) {
            listener.onVideoRenderingStartedMainThread();
        }
    }
    ...
    public void onVideoRenderingStartedMainThread() {
        ...
        notifyOnVideoRenderingStartedMainThread();
    }
    ...
    public void muteVideo() {
        synchronized (mReadyForPlaybackIndicator) {
            PreferenceManager.getDefaultSharedPreferences(getContext())
                    .edit()
                    .putBoolean(IS_VIDEO_MUTED, true)
                    .commit();
    ...
    public void unMuteVideo() {
        synchronized (mReadyForPlaybackIndicator) {
            PreferenceManager.getDefaultSharedPreferences(getContext())
                    .edit()
                    .putBoolean(IS_VIDEO_MUTED, false)
                    .commit();
    ...
    /**
     * @see IjkMediaPlayer#getDuration()
     */
     ...
     *
     * @param position to which to seek to in a video
     */
    ...
    public void updateMuteState(boolean muted) {
        synchronized (mReadyForPlaybackIndicator) {
            if (mMediaPlayer != null) {
                mMediaPlayer.updateMuteState(muted);
            }
        }
    }
}
