...
/**
 * Created by moda on 18/01/19.
 */

/**
 * This class encapsulates {@link IjkMediaPlayer}
 * and follows this use-case diagram:
 * <p>
 * http://developer.android.com/reference/android/media/MediaPlayer.html
 */
public abstract class MediaPlayerWrapper
    implements IjkMediaPlayer.OnErrorListener, IjkMediaPlayer.OnBufferingUpdateListener,
    IjkMediaPlayer.OnInfoListener, IjkMediaPlayer.OnCompletionListener,
  ...
  protected MediaPlayerWrapper(IjkMediaPlayer mediaPlayer) {
    TAG = "" + this;
  ...
  private final Runnable mOnVideoRenderingStartedMessage = new Runnable() {
    @Override
    public void run() {

      //if (SHOW_LOGS) Logger.v(TAG, ">> run, onVideoRenderingStartedMainThread");
      mListener.onVideoRenderingStartedMainThread();
      //if (SHOW_LOGS) Logger.v(TAG, "<< run, onVideoRenderingStartedMainThread");
  ...
  /**
   * This method propagates error when {@link IOException} is thrown during synchronous {@link
   * #prepare()}
   */
  private void onPrepareError(IOException ex) {
    ...
    if (mListener != null) {
      mListener.onErrorMainThread(1,
          -1004); //TODO: remove magic numbers. Find a way to get actual error
    }
    if (mListener != null) {
      mMainThreadHandler.post(new Runnable() {
        @Override
  ...
  /**
   * @see IjkMediaPlayer#setDataSource(Context, Uri)
   */
  public void setDataSource(String filePath, boolean videoMuted) throws IOException {
    synchronized (mState) {
      //if (SHOW_LOGS)
  ...
  /**
   * @see// IjkMediaPlayer#setDataSource(FileDescriptor)
   */
  ...
  private void printInfo(int what) {
    ...
    switch (what) {
      ...
      case IjkMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START:
        ...
        mMainThreadHandler.post(mOnVideoRenderingStartedMessage);
        break;
      ...
      case IjkMediaPlayer.MEDIA_INFO_METADATA_UPDATE:
        ...
        break;
      case IjkMediaPlayer.MEDIA_INFO_UNSUPPORTED_SUBTITLE:
        //if (SHOW_LOGS) Logger.inf(TAG, "onInfo, MEDIA_INFO_UNSUPPORTED_SUBTITLE");

        break;
      case IjkMediaPlayer.MEDIA_INFO_SUBTITLE_TIMED_OUT:
  ...
  private void startPositionUpdateNotifier() {
    ...
    mFuture = mPositionUpdateNotifier.scheduleAtFixedRate(mNotifyPositionUpdateRunnable, 0,
  ...
  public void seekToPosition(long position) {
    synchronized (mState) {
      ...
      //if (SHOW_LOGS) Logger.v(TAG, "seekToPercent, position " + position + ", mState " + state);
      ...
      switch (state) {
        ...
        case END:
        case STOPPED:
          //if (SHOW_LOGS) Logger.w(TAG, "seekToPosition, illegal state");
          break;
  ...
  public void updateMuteState(boolean muted) {
    synchronized (mState) {
      ...
      //if (SHOW_LOGS) Logger.v(TAG, "updateMuteState muted " + muted);
      ...
      switch (state) {
        ...
        case END:
        case STOPPED:
          //if (SHOW_LOGS) Logger.w(TAG, "updateMuteState, illegal state");
          break;
  ...
  public interface MainThreadMediaPlayerListener {
    ...
    void onVideoRenderingStartedMainThread();
  }

  public interface VideoStateListener {
    void onVideoPlayTimeChanged(long positionInMilliseconds);
  }
  //
