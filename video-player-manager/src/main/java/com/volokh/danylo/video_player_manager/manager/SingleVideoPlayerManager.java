...
public class SingleVideoPlayerManager
    implements VideoPlayerManager<MetaData>, VideoPlayerManagerCallback,
  ...
  private static final boolean SHOW_LOGS = true;//Config.SHOW_LOGS;
   ...
   */
  @Override
  public void playNewVideo(MetaData currentItemMetaData, VideoPlayerView videoPlayerView,
      String videoUrl, boolean videoMuted) {
    ...
    if (currentPlayerIsActive) {
      if (isInPlaybackState() && isAlreadyPlayingTheFile) {
        ...
        /** 3. */
        stopAnyPlayback(false);
        startNewPlayback(currentItemMetaData, videoPlayerView, videoUrl, videoMuted);
      } else {
        /** 4. */
        startNewPlayback(currentItemMetaData, videoPlayerView, videoUrl, videoMuted);
      }
  ...
  public void playNewVideo(MetaData currentItemMetaData, VideoPlayerView videoPlayerView,
    ...
    boolean currentPlayerIsActive = mCurrentPlayer == videoPlayerView;
    boolean isAlreadyPlayingTheFile = mCurrentPlayer != null
   ...
   * {@link VideoPlayerView}
   * 3. Set new view player of which become active.
   * 4. Post messages to start new playback
   */
  private void startNewPlayback(MetaData currentItemMetaData, VideoPlayerView videoPlayerView,
      AssetFileDescriptor assetFileDescriptor) {
    // set listener for new player
    // TODO: find a place when we can remove this listener.
    videoPlayerView.addMediaPlayerListener(this);
    //if (SHOW_LOGS) Logger.v(TAG, "startNewPlayback, mCurrentPlayerState " + mCurrentPlayerState+"    "+assetFileDescriptor.getLength());
  ...
  private void startNewPlayback(MetaData currentItemMetaData, VideoPlayerView videoPlayerView,
    ...
    startPlayback(videoPlayerView, videoUrl, videoMuted);
  }
   ...
   * @param videoMuted - whether given muted is muted or not
   */
  private void startPlayback(VideoPlayerView videoPlayerView, String videoUrl, boolean videoMuted) {
    //if(SHOW_LOGS) Logger.v(TAG, "startPlayback");

    mPlayerHandler.addMessages(Arrays.asList(new CreateNewPlayerInstance(videoPlayerView, this),
        new SetUrlDataSourceMessage(videoPlayerView, videoUrl, this, videoMuted),
        new Prepare(videoPlayerView, this), new Start(videoPlayerView, this)));
  }

  private void startPlayback(VideoPlayerView videoPlayerView,
      AssetFileDescriptor assetFileDescriptor) {
    //if(SHOW_LOGS) Logger.v(TAG, "startPlayback");
   ...
   */
  @Override
  public void setVideoPlayerState(VideoPlayerView videoPlayerView,
