...
public class ChangeTracker implements Runnable {
    ...
    enum ChangeTrackerMode {
        OneShot,
        LongPoll,
        Continuous  // does not work, do not use it.
    }

    ChangeTracker(URL databaseURL, ChangeTrackerMode mode, boolean includeConflicts,
     ...
     */
    /* package */
    private String getChangesFeedPath() {
    ...
    private boolean isResponseFailed(Response response) {
        ...
        if (response.code() >= 300 &&
                ((mode == ChangeTracker.ChangeTrackerMode.LongPoll && !Utils.isTransientError(response.code())) ||
                        mode != ChangeTracker.ChangeTrackerMode.LongPoll)) {
    ...
    private boolean isCloudantAuthError(Response response) {
        ...
        if (server == null || !server.contains("CouchDB/"))// (Accurate as of 5/2016)
            return false;
                ...
                try {
                    ...
                    if (responseBody != null) {
                        try {
                            ...
                            if (mode == ChangeTracker.ChangeTrackerMode.LongPoll) {  // continuous replications
