...
public class PreviewGalleryVideoActivity extends AppCompatActivity
    implements FilterAdapter.SelectFilterListner {

  private static final String TAG = PreviewGalleryVideoActivity.class.getSimpleName();
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (ResultHolder.getType() == null || !ResultHolder.getType().equals("video")) {
      //video
    ...
    scaledVideoHeight = ((screenWidth * videoHeight) / videoWidth);

    scaledVideoWidth = screenWidth;

    screenHeight = scaledVideoHeight;
    ...
    if (ResultHolder.getCall().equals("ChatAttachment")) {
      ProgressBarModel progressBarModel = progressBarModels.get(0);
      ...
      if (progressBarModel != null) {
        if (progressBarModel.isVideoItem()) {
          ...
          updateUIStatusToPlaying(0);

          filterEnabled = true;

          rvProgressBars.setVisibility(GONE);
        }
      }
    }
  }
  ...
  public void next() {
    ...
    if (checkIfFiltersAppliedOnAnyVideoSegment()) {
      ...
      progressDialog = new ProgressDialog(PreviewGalleryVideoActivity.this);
  ...
  public void filters() {
    if (filterEnabled) {
      ...
      Toast.makeText(PreviewGalleryVideoActivity.this,
  ...
  private void initializeViewForColorFiltersVideo() {
    ...
    view_filter.setOnTouchListener(new OnSwipeTouchListener(PreviewGalleryVideoActivity.this) {
    ...
    switch (ResultHolder.getCall()) {
      case "story":
        ...
        if (type.equals(Constants.Post.VIDEO)) {
          post.setDub(true);
          post.setDuration(String.valueOf(getDuration(Uri.parse(recordedVideoFiles.get(0)))));
        }
      ...
      case "ChatAttachment":
        ...
        intent = new Intent();
        intent.putExtra("imagePath", recordedVideoFiles.get(0));
  ...
  private void addMediaToBePlayed(String url, int durationInMs, ProgressBarModel progressBarModel) {
    ...
    if (progressBarModel.isFilterSelected()) {
      ...
      gpuPlayerView.setGlFilter(FilterType.createGlFilter(progressBarModel.getSelectedGlFilter(),
          PreviewGalleryVideoActivity.this));
    } else {
  ...
  private void startPlayback() {
    try {
      ...
      if (audioFile != null) {
        player = MediaPlayer.create(PreviewGalleryVideoActivity.this, Uri.parse(audioFile));
  ...
  private void prepareVideoPlaybackFiles(int maximumDuration) {
    ...
    float multiplier = (float) (screenWidth / maximumDuration);
    ...
    rvProgressBars.addOnItemTouchListener(
        new RecyclerItemClickListener(PreviewGalleryVideoActivity.this, rvProgressBars,
    ...
    if (progressBarModel.isFilterSelected()) {
      ...
      if (progressBarModel.getFilterColor().isEmpty()) {
        applyVideoFilter(FilterType.createGlFilter(progressBarModel.getSelectedGlFilter(),
            PreviewGalleryVideoActivity.this), progressBarModel.getVideoPath(), positionOfVideo,
            true, null, true, "");
      } else {
        ...
        applyVideoFilter(FilterType.createGlFilter(progressBarModel.getSelectedGlFilter(),
            PreviewGalleryVideoActivity.this), progressBarModel.getVideoPath(), positionOfVideo,
            true, bitmap, true, progressBarModel.getFilterColor());
      }
    } else {
      if (progressBarModel.getFilterColor().isEmpty()) {
  ...
  private void setupFilterView() {
    ...
    filtersList.addOnItemTouchListener(
        new RecyclerItemClickListener(PreviewGalleryVideoActivity.this, filtersList,
            new RecyclerItemClickListener.OnItemClickListener() {
              @Override
              public void onItemClick(View view, final int position) {
                if (gpuPlayerView != null) {
