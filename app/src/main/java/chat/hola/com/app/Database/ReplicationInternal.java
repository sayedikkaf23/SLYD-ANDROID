...
abstract class ReplicationInternal implements BlockingQueueListener {
    ...
    private static final String BY_CHANNEL_FILTER_NAME = "sync_gateway/bychannel";
    private static final String CHANNELS_QUERY_PARAM = "channels";
    private static final int EXECUTOR_THREAD_POOL_SIZE = 5;
    private static final int MIN_EXECUTOR_THREAD_POOL_SIZE = 2;
    private static final String SYNC_GATEWAY_PREFIX = "Couchbase Sync Gateway/";

    private static int lastSessionID = 0;
    private static int RETRY_DELAY_SECONDS = 60; // #define kRetryDelay 60.0 in CBL_Replicator.m
    ...
    private enum ChangeListenerNotifyStyle {
        SYNC, ASYNC
    }
    ...
    private static int PROCESSOR_DELAY = 500;
    private static int INBOX_CAPACITY = 100;
    ScheduledExecutorService remoteRequestExecutor;
    private Throwable error; // use private to make sure if error is set through setError()
    private String remoteCheckpointDocID;
    private Map<String, Object> remoteCheckpoint;
    AtomicInteger completedChangesCount;
    private AtomicInteger changesCount;
    CollectionUtils.Functor<RevisionInternal, RevisionInternal> revisionBodyTransformationBlock;
    private String sessionID;
    BlockingQueue<Future> pendingFutures;
    ...
    StateMachine<ReplicationState, ReplicationTrigger> stateMachine;
    private final List<ChangeListener> changeListeners = new CopyOnWriteArrayList<ChangeListener>();
    Replication.Lifecycle lifecycle;
    private ReplicationInternal.ChangeListenerNotifyStyle changeListenerNotifyStyle;

    private Future retryFuture = null;  // future obj of retry task

    // for waitingPendingFutures
    boolean waitingForPendingFutures = false;
    final Object lockWaitForPendingFutures = new Object();
    ...
    ReplicationInternal(Database db, URL remote,
        ...
        changeListenerNotifyStyle = ReplicationInternal.ChangeListenerNotifyStyle.ASYNC;
     ...
     */
    void fireTrigger(final ReplicationTrigger trigger) {
    ...
    private void initializeReplicationExecutor() {
        if (executor == null) {
    ...
    private void initializeRequestWorkers() {
    ...
    @InterfaceAudience.Private
    private void checkSessionAtPath(final String sessionPath) {
    ...
    AtomicInteger getCompletedChangesCount() {
     ...
     */
    @InterfaceAudience.Private
    CustomFuture sendAsyncRequest(String method, String relativePath,
                                  Map<String, ?> body,
    ...
    @InterfaceAudience.Private
    private CustomFuture sendAsyncRequest(String method, String relativePath,
     ...
     */
    @InterfaceAudience.Private
    private CustomFuture sendAsyncRequest(String method,
                                          URL url,
                                          boolean cancelable,
                                          Map<String, ?> body,
                                          boolean dontLog404,
     ...
     */
    @InterfaceAudience.Private
    CustomFuture sendAsyncMultipartRequest(String method,
                                           String relativePath,
    ...
    private void setLastSequenceFromWorkExecutor(final String lastSequence, final String checkpointId) {
    ...
    private String remoteCheckpointDocID(String localUUID) {
     ...
     */
    ReplicationFilter compilePushReplicationFilter() {
        if (isPull())
            return null;
     ...
     */
    boolean canSendCompressedRequests() {
    ...
    private void notifyChangeListeners(final Replication.ChangeEvent changeEvent) {
        if (changeListenerNotifyStyle == ReplicationInternal.ChangeListenerNotifyStyle.SYNC) {
            for (ReplicationInternal.ChangeListener changeListener : changeListeners) {
                try {
     ...
     */
    /* package */ void addChangeListener(ReplicationInternal.ChangeListener changeListener) {
    ...
    @InterfaceAudience.Private
    private boolean serverIsSyncGateway() {
        return !(serverType == null || !serverType.startsWith(SYNC_GATEWAY_PREFIX));
    }

    @InterfaceAudience.Private
    private static boolean serverIsSyncGatewayVersion(String serverName, String minVersion) {
        if (serverName == null) {
            return false;
        } else {
     ...
     */
    @InterfaceAudience.Private
    void addToInbox(RevisionInternal rev) {
     ...
     */
    private void retryReplicationIfError() {
    ...
    void setLifecycle(Replication.Lifecycle lifecycle) {
        this.lifecycle = lifecycle;
    }
     ...
     */
    @InterfaceAudience.Private
    void setLastSequence(String lastSequenceIn) {
    ...
    RevisionInternal transformRevision(RevisionInternal rev) {
    ...
    @InterfaceAudience.Private
    static Status statusFromBulkDocsResponseItem(Map<String, Object> item) {
     ...
     */
    static String encodeDocumentId(String docId) {
    ...
    void waitBatcherCompleted() {
        waitBatcherCompleted(batcher);
    }

    static void waitBatcherCompleted(Batcher<RevisionInternal> b) {
     ...
     void waitPendingFuturesCompleted() {
        try {
            while (!pendingFutures.isEmpty()) {
