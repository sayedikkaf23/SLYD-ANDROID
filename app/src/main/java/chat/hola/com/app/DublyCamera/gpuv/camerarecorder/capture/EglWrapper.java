...
//        if (DEBUG) Log.d(TAG, "makeDefault:");
        if (!EGL14.eglMakeCurrent(eglDisplay, EGL14.EGL_NO_SURFACE, EGL14.EGL_NO_SURFACE, EGL14.EGL_NO_CONTEXT)) {
            Log.e("TAG", "makeDefault" + EGL14.eglGetError());
        }
    }
...
//		if (DEBUG) Log.d(TAG, "swap:");
        if (!EGL14.eglSwapBuffers(eglDisplay, surface)) {
            ...
            if (DEBUG) Log.e(TAG, "swap:err=" + err);
            return err;
        }
...
//        if (DEBUG) Log.d(TAG, "createWindowSurface:nativeWindow=" + surface);
