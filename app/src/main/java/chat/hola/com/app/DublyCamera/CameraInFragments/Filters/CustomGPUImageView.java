...
public class CustomGPUImageView extends FrameLayout {

    private GLSurfaceView mGLSurfaceView;
    private CustomGPUImage mGPUImage;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mGPUImage = new CustomGPUImage(getContext());
     ...
     */
    public CustomGPUImage getGPUImage() {
        return mGPUImage;
    }
     ...
     */
    public void setScaleType(CustomGPUImage.ScaleType scaleType) {
...
//                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
            File path =
                    AppController.getInstance().getExternalFilesDir(null);
    ...
    public void saveToSpecifiedFolder(final String folderPath, final String fileName,
                                      int width, int height,
                                      final OnPictureSavedListener listener) {
        new SaveToFolderTask(folderPath, fileName, width, height, listener).execute();
    }

    public void saveToSpecifiedFolder(final String folderPath, final String fileName,
                                      final OnPictureSavedListener listener) {
        new SaveToFolderTask(folderPath, fileName, listener).execute();
    }

    private class SaveToFolderTask extends AsyncTask<Void, Void, Void> {
        ...
        public SaveToFolderTask(final String folderName, final String fileName,
                                final OnPictureSavedListener listener) {
            this(folderName, fileName, 0, 0, listener);
        }

        public SaveToFolderTask(final String folderPath, final String fileName, int width, int height,
                                final OnPictureSavedListener listener) {
        ...
        protected Void doInBackground(final Void... params) {
            try {
                Bitmap result = mWidth != 0 ? capture(mWidth, mHeight) : capture();
                saveImage(mFolderPath, mFileName, result);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            return null;
        ...
        private void saveImage(final String folderPath, final String fileName, final Bitmap image) {
