...
public class PaidPresenter implements PaidContract.Presenter {
    ...
    /*
     * Bug Title: needs to show posts that are paid on the creators profile and not posts I have bought
     * Bug Id: #2789
     * Fix Desc; api
     * Fix Dev: Hardik
     * Fix Date: 24/6/21
    ...
    public void loadData(int skip, int limit, boolean isLiked) {
        ...
        service.getPaidPost(AppController.getInstance().getApiToken(), Constants.LANGUAGE, userId, skip,
                limit)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
