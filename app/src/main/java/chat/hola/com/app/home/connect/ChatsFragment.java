...
import chat.hola.com.app.TopSnackBar.TSnackbar;
...
public class ChatsFragment extends Fragment implements View.OnClickListener,
        ClickListner,
        ChatlistAdapter.OnButtonClickListener,
        ItemDragSwipeCallback.OnTouchListener,
        ChatlistAdapter.StartDragListener {
...
//        connectFragment.setSearchListner(this);
        ...
        mAdapter = new ChatlistAdapter(getActivity(), mChatData,this,this,this);
        recyclerView_chat.setLayoutManager(new CustomLinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false));
        recyclerView_chat.setItemAnimator(new DefaultItemAnimator());
        recyclerView_chat.setAdapter(mAdapter);

        ItemTouchHelper.Callback callback = new ItemMoveCallback(getContext(),R.color.red,R.drawable.ic_delete,mAdapter);
        touchHelper = new ItemTouchHelper(callback);
        touchHelper.attachToRecyclerView(recyclerView_chat);
...
//                        mAdapter.notifyItemInserted(mProductData.size() - 1);
            ...
            if (map != null) {
                ...
                for (int i = 0; i < receiverUidArray.size(); i++) {
                    Map<String, Object> chatInfo =  db.getParticularChatInfo(receiverDocIdArray.get(i),false);
                   if(chatInfo != null){
                       chats.add(chatInfo);
...
//                    searchView.clearFocus();
    ...
    public void requestDrag(RecyclerView.ViewHolder viewHolder) {
        com.couchbase.lite.util.Log.e("RequestDrag", "====>" + viewHolder.getItemViewType());
        touchHelper.startDrag(viewHolder);
    }

    @Override
    public void onDeleteButtonClick(int position) {
        com.couchbase.lite.util.Log.e("onDeleteClick", "===>>" + position);
    ...
    @Override
    public void onItemClickFrame(int position) {
            ...
            for (int i = 0; i < mSearchableData.size(); i++) {
                ...
                try {
                    ...
                    if ((mSearchableData.get(i).getReceiverName().toLowerCase(Locale.US)).contains(searchText)) {
                        ...
                        mReceiverName = mSearchableData.get(i).getDocumentId();
                        if(!mSearchableData.get(i).getDocumentId().equals(mReceiverName)){
