...
public class PullerInternal extends ReplicationInternal implements ChangeTrackerClient {
    ...
    static final int MAX_NUMBER_OF_ATTS_SINCE = 50;

    // Tune this parameter based on application needs.
    private static int CHANGE_TRACKER_RESTART_DELAY_MS = 10 * 1000;

    private static final int MAX_PENDING_DOCS = 200;

    private static final int INSERTION_BATCHER_DELAY = 250; // 0.25 Seconds

    private ChangeTracker changeTracker;
    private SequenceMap pendingSequences;
    private Boolean canBulkGet;  // Does the server support _bulk_get requests?
    private List<RevisionInternal> revsToPull = Collections.synchronizedList(
            new ArrayList<RevisionInternal>(100));
    private List<RevisionInternal> bulkRevsToPull = Collections.synchronizedList(
            new ArrayList<RevisionInternal>(100));
    private List<RevisionInternal> deletedRevsToPull = Collections.synchronizedList(
            new ArrayList<RevisionInternal>(100));
    private int httpConnectionCount;
    private Batcher<RevisionInternal> downloadsToInsert;
    ...
    private void pullBulkRevisions(List<RevisionInternal> bulkRevs) {
    ...
    @SuppressWarnings("unchecked")
    private void pullBulkWithAllDocs(final List<RevisionInternal> bulkRevs) {
     ...
     */
    @InterfaceAudience.Private
    private void insertDownloads(final List<RevisionInternal> downloads) {
    ...
    @InterfaceAudience.Private
    @SuppressWarnings("unchecked")
    private void pullRemoteRevision(final RevisionInternal rev) {
    ...
    @InterfaceAudience.Private
    private static String joinQuotedEscaped(List<String> strings) {
        if (strings.size() == 0) {
            return "[]";
        }
        byte[] json = null;
     ...
     */
    @SuppressWarnings("unchecked")
    private void processChangeTrackerChange(final Map<String, Object> change) {
    ...
    public void changed(BlockingQueueListener.EventType type, Object o, BlockingQueue queue) {
        if ((type == BlockingQueueListener.EventType.PUT || type == BlockingQueueListener.EventType.ADD) &&
                isContinuous() &&
                !queue.isEmpty()) {
    ...
    private void waitDownloadsToInsertBatcherCompleted() {
