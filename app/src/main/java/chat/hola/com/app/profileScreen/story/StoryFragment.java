...
public class StoryFragment extends DaggerFragment
        implements StoryContract.View, TrendingDtlAdapter.ClickListner {
    ...
    private TrendingDtlAdapter adapter;
    @Inject
    BlockDialog dialog;
    ...
    public void loadData() {
        if (userId != null && !userId.equals("")) {
            if (userId.equals(AppController.getInstance().getUserId())) {
                ...
                presenter.loadMemberData(userId, StoryPresenter.page * StoryPresenter.PAGE_SIZE,
                        StoryPresenter.PAGE_SIZE, type.equals("nogrid"));
            }
        }
    }
    ...
    /*
     * Bug Title: need to reload the app to see my post that I just created
     * Bug Id: #2763
     * Fix Dev: Hardik
     * Fix Desc: bus event
     * Fix Date: 23/6/21
    ...
    public void getMessage(JSONObject object) {
        try {
            if (object.getString("eventName").equals("postCompleted")) {
                /*used to refresh when current user posted something*/
                StoryPresenter.page = 0;
                loadData();
            }
