...
package io.isometrik.android.rtmp.encoder;
...
import io.isometrik.android.rtmp.encoder.gles.EglCore;
import io.isometrik.android.rtmp.encoder.gles.FullFrameRect;
import io.isometrik.android.rtmp.encoder.gles.Texture2dProgram;
import io.isometrik.android.rtmp.encoder.gles.WindowSurface;
...
public class TextureMovieEncoder implements Runnable {
  ...
  public void frameAvailable(SurfaceTexture st) {
    ...
    if (mVideoEncoder != null && (frameTime - mLastFrameTime) >= getFrameInterval()) {
      ...
      long timestamp = (frameTime - mRecordingStartTime) * 1000000; // convert it to nano seconds
  ...
  private void changeEffect(Texture2dProgram.ProgramType type) {
    ...
    mFullScreen = new FullFrameRect(new Texture2dProgram(type));
    mProgramType = type;
  }
  ...
  private void handleStartRecording(EncoderConfig config) {
    ...
    prepareEncoder(config.mEglContext, config.mWidth, config.mHeight, config.mBitRate,
        config.mFrameRate, config.writerHandler, config.mProgramType);
  }
  ...
  private void prepareEncoder(EGLContext sharedContext, int width, int height, int bitRate,
      int frameRate, IMediaMuxer writerHandle, Texture2dProgram.ProgramType programType)
    ...
    mProgramType = programType;
    mFullScreen = new FullFrameRect(new Texture2dProgram(programType));
  }
