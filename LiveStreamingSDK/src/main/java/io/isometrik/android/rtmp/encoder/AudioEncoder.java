...
public class AudioEncoder extends Thread {
   ...
   */
  public boolean startAudioEncoder(int sampleRate, int channelCount, int bitrate, int maxInputSize,
    ...
    try {
      ...
      mAudioEncoder.configure(audioFormat, null /* surface */, null /* crypto */,
  ...
  public void stopEncoding() {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (inputBufferId >= 0) {
        mAudioEncoder.queueInputBuffer(inputBufferId, 0, 0, 0,
  ...
  public void run() {
    ...
    for (; ; ) {
      ...
      if (outputBufferId >= 0) {
        ...
        long presentationTimeInMillis = (roundOffset + getUnsignedInt(info.presentationTimeUs))
        ...
        byte[] data =
            getBuffer(info.size, mMuxerHandler.getLastAudioFrameTimeStamp(), presentationTime);
