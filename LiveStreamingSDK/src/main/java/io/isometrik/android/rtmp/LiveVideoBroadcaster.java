package io.isometrik.android.rtmp;
...
import io.isometrik.android.rtmp.encoder.AudioHandler;
import io.isometrik.android.rtmp.encoder.CameraSurfaceRenderer;
import io.isometrik.android.rtmp.encoder.TextureMovieEncoder;
import io.isometrik.android.rtmp.encoder.VideoEncoderCore;
import io.isometrik.android.rtmp.network.IMediaMuxer;
import io.isometrik.android.rtmp.network.RTMPStreamer;
import io.isometrik.android.rtmp.utils.Resolution;
import io.isometrik.android.rtmp.utils.Utils;
...
public class LiveVideoBroadcaster extends Service
    ...
    public void onDestroy() {
        ...
        if (mRtmpHandlerThread != null) mRtmpHandlerThread.quitSafely();
    ...
    public boolean startBroadcasting(String rtmpUrl) {
        ...
        try {
            ...
            if (result) {
                ...
                audioThread =
                        new AudioRecorderThread(SAMPLE_AUDIO_RATE_IN_HZ, recordStartTime, audioHandler);
                audioThread.start();
                isRecording = true;
    ...
    public void openCamera(int cameraId, final int width, final int height) {
        //check permission
        ...
        if (cameraId == Camera.CameraInfo.CAMERA_FACING_FRONT && !getPackageManager().hasSystemFeature(
        ...
        new AsyncTask<Integer, Void, Camera.Parameters>() {
            ...
            protected Camera.Parameters doInBackground(Integer... params) {
                ...
                try {
                    ...
                    do {
                        ...
                        //   setResolution(new Resolution(screenWidth, screenHeight));
                    ...
                    if (sCameraProxy.isCameraAvailable()) {
                        ...
                        if (parameters != null) {
                            setCameraParameters(parameters, width, height);

                            if (Utils.doesEncoderWorks(context) == Utils.ENCODER_NOT_TESTED) {
                                boolean encoderWorks =
                                        VideoEncoderCore.doesEncoderWork(previewSize.width, previewSize.height, 200000,
                                                20);
            ...
            protected void onPostExecute(Camera.Parameters parameters) {
                if (context.isFinishing()) {
                    ...
                    Snackbar.make(mGLView, R.string.camera_not_running_properly, Snackbar.LENGTH_LONG).show();
                }
            }
    ...
    private int setCameraParameters(Camera.Parameters parameters, int width, int height) {
    ...
    public boolean isPermissionGranted() {
        ...
        boolean mFilePermissionGranted =
                ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                        == PackageManager.PERMISSION_GRANTED;

        return cameraPermissionGranted && microPhonePermissionGranted && mFilePermissionGranted;
    }
    ...
    public void requestPermission() {
        ...
        if (!mFilePermissionGranted) {
            permissionList.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
        }
        if (permissionList.size() > 0) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(context,
                    Manifest.permission.CAMERA)) {
                mAlertDialog = new AlertDialog.Builder(context).setTitle(R.string.permission)
                ...
                mAlertDialog = new AlertDialog.Builder(context).setMessage(
                ...
                mAlertDialog = new AlertDialog.Builder(context).setMessage(
                        getString(R.string.file_permission_is_required))
                        .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                String[] permissionArray =
                                        permissionList.toArray(new String[permissionList.size()]);
    ...
    public int[] findBestFrameRate(List<int[]> frameRateList, int[] requestedFrameRate) {
        ...
        for (int i = 1; i < size; i++) {
            ...
            if (Math.abs(requestedAverage - bestRateAverage) >= Math.abs(
                    requestedAverage - rateAverage)) {

                if ((Math.abs(requestedFrameRate[0] - rate[0]) <= Math.abs(
                        requestedFrameRate[0] - bestRate[0])) || (Math.abs(requestedFrameRate[1] - rate[1])
