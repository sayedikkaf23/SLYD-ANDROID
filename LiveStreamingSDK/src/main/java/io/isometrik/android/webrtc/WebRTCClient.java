...
import io.isometrik.android.R;
import io.isometrik.android.webrtc.apprtc.AppRTCAudioManager;
import io.isometrik.android.webrtc.apprtc.AppRTCClient;
import io.isometrik.android.webrtc.apprtc.Constants;
import io.isometrik.android.webrtc.apprtc.PeerConnectionClient;
import io.isometrik.android.webrtc.apprtc.ProxyVideoSink;
import io.isometrik.android.webrtc.recorder.RecorderSurfaceDrawer;
 ...
 * and call view.
 */
public class WebRTCClient implements IWebRTCClient, AppRTCClient.SignalingEvents,
    ...
    private boolean surfaceNotInitializedOnce = true;
    ...
    //public WebRTCClient(IWebRTCListener webRTCListener, Context context,
    //    WebRtcAudioRecord.IAudioRecordStatusListener audioRecordStatusListener) {
    //  this.webRTCListener = webRTCListener;
    //  this.context = context;
    ...
    @Override
    public void init(String url, String streamId, String mode, String token, Intent intent, boolean participantList) {
        ...
        if (saveRemoteVideoToFile != null) {
            try {
                videoFileRenderer =
                        new VideoFileRenderer(saveRemoteVideoToFile, videoOutWidth, videoOutHeight,
                                eglBase.getEglBaseContext());
                remoteSinks.add(videoFileRenderer);
            } catch (IOException e) {
                throw new RuntimeException("Failed to open video file for output: " + saveRemoteVideoToFile,
                        e);
            }
        }
        ...
        fullscreenRenderer.setZOrderMediaOverlay(participantList);
        ...
        boolean loopback = intent.getBooleanExtra(Constants.EXTRA_LOOPBACK, false);
        boolean tracing = intent.getBooleanExtra(Constants.EXTRA_TRACING, false);

//        int videoWidth = intent.getIntExtra(Constants.EXTRA_VIDEO_WIDTH, 0);
//        int videoHeight = intent.getIntExtra(Constants.EXTRA_VIDEO_HEIGHT, 0);
...
//        if (videoHeight == 0)
        ...
        screencaptureEnabled = intent.getBooleanExtra(Constants.EXTRA_SCREENCAPTURE, false);
...
//        }
        ...
        if (intent.getBooleanExtra(Constants.EXTRA_DATA_CHANNEL_ENABLED, false)) {
            dataChannelParameters = new PeerConnectionClient.DataChannelParameters(
                    intent.getBooleanExtra(Constants.EXTRA_ORDERED, true),
                    intent.getIntExtra(Constants.EXTRA_MAX_RETRANSMITS_MS, -1),
                    intent.getIntExtra(Constants.EXTRA_MAX_RETRANSMITS, -1),
                    intent.getStringExtra(Constants.EXTRA_PROTOCOL),
                    intent.getBooleanExtra(Constants.EXTRA_NEGOTIATED, false),
                    intent.getIntExtra(Constants.EXTRA_ID, -1));
        }

        String videoCodec = "VP8";//intent.getStringExtra(Constants.EXTRA_VIDEOCODEC);

//        int videoStartBitrate =  this.intent.getIntExtra(Constants.EXTRA_VIDEO_BITRATE, 0);
//        if (videoStartBitrate == 0) {
        int videoStartBitrate = Integer.parseInt(this.context.getString(R.string.pref_maxvideobitratevalue_default));
//1700
        //        }

//        int audioStartBitrate = this.intent.getIntExtra(Constants.EXTRA_AUDIO_BITRATE, 0);
//        if (audioStartBitrate == 0) {
        int audioStartBitrate = Integer.parseInt(this.context.getString(R.string.pref_startaudiobitratevalue_default));
//        }

        boolean videoCallEnabled = intent.getBooleanExtra(Constants.EXTRA_VIDEO_CALL, true);
        ...
        Log.i(TAG, "videoCodec: " + videoCodec);
        Log.i(TAG, "videoStartBitrate: " + videoStartBitrate);
        Log.i(TAG, "audioStartBitrate: " + audioStartBitrate);
        peerConnectionParameters =
                new PeerConnectionClient.PeerConnectionParameters(videoCallEnabled, loopback, tracing,
                        videoWidth, videoHeight, intent.getIntExtra(Constants.EXTRA_VIDEO_FPS, 0),
                        videoStartBitrate, videoCodec,
                        intent.getBooleanExtra(Constants.EXTRA_HWCODEC_ENABLED, true),
                        intent.getBooleanExtra(Constants.EXTRA_FLEXFEC_ENABLED, false), audioStartBitrate,
                        intent.getStringExtra(Constants.EXTRA_AUDIOCODEC),
                        intent.getBooleanExtra(Constants.EXTRA_NOAUDIOPROCESSING_ENABLED, false),
                        intent.getBooleanExtra(Constants.EXTRA_AECDUMP_ENABLED, false),
                        intent.getBooleanExtra(Constants.EXTRA_SAVE_INPUT_AUDIO_TO_FILE_ENABLED, false),
                        intent.getBooleanExtra(Constants.EXTRA_OPENSLES_ENABLED, false),
                        intent.getBooleanExtra(Constants.EXTRA_DISABLE_BUILT_IN_AEC, false),
                        intent.getBooleanExtra(Constants.EXTRA_DISABLE_BUILT_IN_AGC, false),
                        intent.getBooleanExtra(Constants.EXTRA_DISABLE_BUILT_IN_NS, false),
                        intent.getBooleanExtra(Constants.EXTRA_DISABLE_WEBRTC_AGC_AND_HPF, false),
                        intent.getBooleanExtra(Constants.EXTRA_ENABLE_RTCEVENTLOG, false),
                        intent.getBooleanExtra(Constants.EXTRA_USE_LEGACY_AUDIO_DEVICE, false),
                        dataChannelParameters, audioCallEnabled);
        commandLineRun = intent.getBooleanExtra(Constants.EXTRA_CMDLINE, false);
        int runTimeMs = intent.getIntExtra(Constants.EXTRA_RUNTIME, 0);

        Log.d(TAG, "VIDEO_FILE: '" + intent.getStringExtra(Constants.EXTRA_VIDEO_FILE_AS_CAMERA) + "'");

        // Create connection parameters.
        String urlParameters = intent.getStringExtra(Constants.EXTRA_URLPARAMETERS);
        ...
        peerConnectionClient = new PeerConnectionClient(this.context.getApplicationContext(), eglBase,
                peerConnectionParameters, WebRTCClient.this);
        //peerConnectionClient.setAudioRecordStatusListener(audioRecordStatusListener);
        //peerConnectionClient.setAudioRecordStatusListener(null);
    ...
    //      surfaceNotInitializedOnce = false;
    ...
    //
    //  if (peerConnectionClient.getSurfaceWidth() == 0
    //      || peerConnectionClient.getSurfaceHeight() == 0) {
    //    throw new IllegalArgumentException(
    //        "Surface is not initialized. Surface width or height is zero. "
    ...
    //    handlerThread.start();
    //    recorderSurfaceDrawer =
    //        new RecorderSurfaceDrawer(getEglBase(), handlerThread.getLooper(), videoBitrate, file,
    //            peerConnectionClient.getWebRtcAudioRecord(), peerConnectionClient.getSampleRate(), 1);
    //
    //    recorderSurfaceDrawer.startRecording(peerConnectionClient.getSurfaceWidth(),
    //        peerConnectionClient.getSurfaceHeight());
    //
    ...
    public void startStream() {
        ...
        appRtcClient = new WebSocketRTCIsometrikClient(this);
    ...
    private void startScreenCapture() {
        ...
        if (this.context instanceof Activity) {
            ((Activity) this.context).startActivityForResult(
                    ...
                    Constants.CAPTURE_PERMISSION_REQUEST_CODE);
        }
    }

    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != Constants.CAPTURE_PERMISSION_REQUEST_CODE) return;
    ...
    private boolean useCamera2() {
        return Camera2Enumerator.isSupported(this.context) && this.intent.getBooleanExtra(
                Constants.EXTRA_CAMERA2, true);
    }

    private boolean captureToTexture() {
        return this.intent.getBooleanExtra(Constants.EXTRA_CAPTURETOTEXTURE_ENABLED, true);
    }
    ...
    // StreamingControls.OnCallEvents interface implementation.
    @Override
    ...
    @Override
    public void switchCamera(boolean rearCameraActive) {
        if (peerConnectionClient != null) {
            if (fullscreenRenderer != null) {
                fullscreenRenderer.setMirror(rearCameraActive);
            }
            peerConnectionClient.switchCamera(rearCameraActive);
        }
    }
    ...
    //        this.context.getText(R.string.channel_error_title))
...
//    logToast.show();
    ...
    VideoCapturer createVideoCapturer() {
        ...
        String videoFileAsCamera = this.intent.getStringExtra(Constants.EXTRA_VIDEO_FILE_AS_CAMERA);
    ...
    @Override
    public void setVideoRenderers(SurfaceViewRenderer pipRenderer,
